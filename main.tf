...
resource "aws_subnet" "public_subnet" {
  ...
  availability_zone = data.aws_availability_zones.available.names[count.index]    # so these two subnets are in idfferent AZ
  tags = merge(
    local.default_tags, {
...
# Two subnets in different Az can connect to one Route table
